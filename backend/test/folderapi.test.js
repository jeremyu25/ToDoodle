...
describe("Folder API", () => {
  describe("GET /api/v1/folder", () => {
    it("Should return 400 if folder id is missing", async () => {
      const res = await request(app).get("/api/v1/folder")
      expect(res.status).toBe(400)
      expect(res.body.message).toEqual("Folder ID is required.")
    })

    it("Should return 404 if folder not found", async () => {
      FolderModel.getFolderById.mockResolvedValueOnce(null)
      const res = await request(app).get("/api/v1/folder").query({ id: "nonexistent-id" })
    ...
    it("Should return 200 with folder data if found", async () => {
      ...
      const res = await request(app).get("/api/v1/folder").query({ id: "folder1" })
  ...
  describe("GET /api/v1/folder/all", () => {
    it("Should return 400 if user_id is missing", async () => {
      const res = await request(app).get("/api/v1/folder/all")
      expect(res.status).toBe(400)
    ...
    it("Should return 200 with empty array if no folders found", async () => {
      ...
      const res = await request(app).get("/api/v1/folder/all").query({ user_id: "user123" })
      expect(res.status).toBe(200)
  ...
  describe("POST /api/v1/folder", () => {
    it("Should return 400 if user_id or name is missing", async () => {
      const res = await request(app).post("/api/v1/folder").query({ user_id: "user123" })
      expect(res.status).toBe(400)
      expect(res.body.message).toEqual("User ID and folder name are required.")
    })
    ...
    it("Should return 201 when folder is created", async () => {
      ...
      const res = await request(app)
        .post("/api/v1/folder")
        .query({ user_id: "user123", name: "New Folder", description: "Desc" })
  ...
  describe("PATCH /api/v1/folder/name", () => {
    it("Should return 400 if id or name is missing", async () => {
      const res = await request(app).patch("/api/v1/folder/name").query({ id: "folder1" })
      expect(res.status).toBe(400)
    ...
    it("Should return 404 if folder not found", async () => {
      ...
      const res = await request(app).patch("/api/v1/folder/name").query({ id: "nonexistent", name: "Renamed" })
      expect(res.status).toBe(404)
      expect(res.body.message).toEqual("User folder not found.")
    })
    ...
    it("Should return 200 when folder name is updated", async () => {
      ...
      const res = await request(app).patch("/api/v1/folder/name").query({ id: "folder1", name: "Renamed Folder" })
      expect(res.status).toBe(200)
      expect(res.body.status).toEqual("success")
  ...
  describe("PATCH /api/v1/folder/description", () => {
    it("Should return 400 if id or description is missing", async () => {
      const res = await request(app).patch("/api/v1/folder/description").query({ id: "folder1" })
      expect(res.status).toBe(400)
    ...
    it("Should return 404 if folder not found", async () => {
      ...
      const res = await request(app).patch("/api/v1/folder/description").query({ id: "nonexistent", description: "New Desc" })
      expect(res.status).toBe(404)
      expect(res.body.message).toEqual("User folder not found.")
    })
    ...
    it("Should return 200 when folder description is updated", async () => {
      ...
      const res = await request(app).patch("/api/v1/folder/description").query({ id: "folder1", description: "Updated Desc" })
      expect(res.status).toBe(200)
      expect(res.body.status).toEqual("success")
      expect(res.body.data).toEqual(mockUpdated)
    })
  ...
  describe("DELETE /api/v1/folder", () => {
    it("Should return 404 if folder not found", async () => {
      ...
      const res = await request(app).delete("/api/v1/folder").query({ id: "nonexistent" })
      expect(res.status).toBe(404)
      expect(res.body.message).toEqual("User folder not found.")
    })
    ...
    it("Should return 200 when folder is deleted", async () => {
      ...
      const res = await request(app).delete("/api/v1/folder").query({ id: "folder1" })
      expect(res.status).toBe(200)
      expect(res.body.data).toEqual(mockDeleted)
    })
  })

  describe("DELETE /api/v1/folder/all", () => {
    it("Should return 404 if no folders found", async () => {
      FolderModel.deleteAllFolders.mockResolvedValueOnce([])
      const res = await request(app).delete("/api/v1/folder/all").query({ user_id: "user123" })
      expect(res.status).toBe(404)
